{ ... }: {

# ██████╗  ██████╗ ███████╗██╗
# ██╔══██╗██╔═══██╗██╔════╝██║
# ██████╔╝██║   ██║█████╗  ██║
# ██╔══██╗██║   ██║██╔══╝  ██║
# ██║  ██║╚██████╔╝██║     ██║
# ╚═╝  ╚═╝ ╚═════╝ ╚═╝     ╚═╝

  # Imports all configuration files for theme "lawson".
  xdg.configFile."rofi/font.rasi".source = ./font.rasi;
  xdg.configFile."rofi/colors.rasi".source = ./colors.rasi;
  xdg.configFile."rofi/confirm.rasi".source = ./confirm.rasi;
  xdg.configFile."rofi/askpass.rasi".source = ./askpass.rasi;
  xdg.configFile."rofi/launcher.rasi".source = ./launcher.rasi;
  xdg.configFile."rofi/powermenu.rasi".source = ./powermenu.rasi;

}
